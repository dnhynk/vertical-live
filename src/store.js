...
const usePetStore = create((set, get) => ({
    ...
    dispatchEvent: (rawEvent) => {
        ...
        const canRevive = ['REVIVE', 'SUPER_CHAT', 'SUPER_STICKER', 'GIFT', 'JEWEL_GIFT'].includes(event.type);
        ...
        switch (event.type) {
            ...
            case 'FEED':
                ...
                break;
            case 'SUPER_STICKER':
                gain = { hunger: 12 + tier * 4, happiness: 10 + tier * 4, cleanliness: 3, life: 6, evolutionXp: 8 + tier * 3 };
                action = tier >= 2 ? 'Dance' : 'Cheer';
                label = event.amount?.display || 'Super Sticker';
                duration = 3200;
                break;
            case 'SUPER_CHAT':
                gain = { hunger: 18 + tier * 8, happiness: 16 + tier * 8, cleanliness: 5, life: 12 + tier * 8, evolutionXp: 15 + tier * 8 };
            ...
            case 'GIFT':
            case 'JEWEL_GIFT':
                gain = { hunger: 20 + tier * 10, happiness: 20 + tier * 10, cleanliness: 8, life: 18 + tier * 10, evolutionXp: 20 + tier * 10 };
                action = 'BigGift';
        ...
        const nextHappiness = revived ? Math.max(30, gain.happiness) : clampStat(state.happiness + gain.happiness);
