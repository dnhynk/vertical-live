 ...
 *     `command` or `command <short token>`, and the token may not itself be a
 *     command; the token charset is the contract's (`CommandRefSchema`), so
 *     nothing longer can be smuggled through it.
