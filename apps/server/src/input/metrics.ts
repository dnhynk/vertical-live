 ...
 * command. Both counters are anonymous integers — there is no per-author
 * breakdown to build while `actor` is `null` (spec §7.4, BOARD A-1), and no
 * message text is retained, so a rejection contributes only its code.
 *
...
export interface CommandMetricsSnapshot {
  ...
  /** `accepted / commandLike`, or `null` before any command-like message. */
...
export class CommandMetrics {
  ...
  snapshot(): CommandMetricsSnapshot {
    return {
      ...
      commandSuccessRatio: this.#commandLike === 0 ? null : this.#accepted / this.#commandLike,
    }
  }
