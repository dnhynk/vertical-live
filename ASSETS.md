...
- 지금 화면에 쓰이는 시각 자산은 **전부 코드로 만든 오리지널**이다. 외부에서 받은 이미지·모델·폰트·아이콘 세트는 하나도 없다(번들에 포함되는 파일은 `favicon.svg` 하나뿐).
- 음향은 아직 없다. 소리를 넣을 때도 같은 규칙(자체 제작 또는 CC0·상업 허용)을 적용하고 이 표에 적는다.
