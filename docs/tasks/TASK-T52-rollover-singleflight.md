...
4. Add focused crash-boundary tests for pre-stop and post-stop rollover interruption plus the exact production topology of a newest interrupted replacement, an older rollover attempt, and an oldest live attempt. Assert stable resource counts, fixed stop-before-new-live ordering, preserved public/privacy and chat target behavior, and observed background rollover failures.
