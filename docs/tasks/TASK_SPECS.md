...
- **구현 계약**:
  ...
  - due rollover는 기존 순서를 그대로 지킨다: 같은 OBS ingestion stream에 replacement create/bind → predecessor `complete` → replacement `live`. OBS stop/restart, world snapshot/inbox/checkpoint mutation, publish/privacy 우회는 없다. chat resolver는 serialization 뒤 최신 open binding을 읽어 새 `liveChatId`로 retarget한다.
  ...
  - newest replacement가 `bound`이고 정확히 하나의 older live predecessor가 있으면 이는 중단된 rollover로 복구한다: predecessor를 먼저 `complete`(unknown outcome이면 reconcile)하고 replacement를 `live`로 만든다. predecessor가 이미 terminal이면 근거를 기록하고 replacement를 이어간다. 둘 이상의 competing live 상태, incomplete list/unknown lifecycle, binding 불일치처럼 정당 owner를 증명할 수 없으면 insert/repoint/close로 추측하지 않고 기존 safe-stop/alert 경로로 올린다.
...
- **합격 기준**
  ...
  2. 모든 public lifecycle mutation의 단일 serialization, 실패 뒤 queue 진행, composite operation의 non-reentrant 성질을 focused test로 증명한다. rollover의 bind→old complete→new live 순서, OBS 무중단, publish/privacy, chat retarget이 회귀하지 않는다.
  3. exact production topology(새est interrupted replacement + older rollover attempt + oldest live attempt)와 rollover crash boundaries(새 replacement bind 전/후, predecessor stop 전/후, replacement live 전/후)를 재시작해 검증한다. newest legitimate state를 결정론적으로 resume하고, terminal/missing evidence가 있는 stale row만 reason과 함께 닫으며, ambiguous state는 resource 생성·ID rewrite 없이 safe-stop/alert한다.
