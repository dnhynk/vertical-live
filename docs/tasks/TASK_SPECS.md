...
- **범위**
  - **`invalid`과 `unsupported`를 나눈다.** `unsupported`는 정상 트래픽이다(계약이 모든 YouTube 메시지 타입을 모델링하지 않는다). `invalid`는 **계약이 플랫폼이 보낸 것을 읽지 못했다**는 뜻이고 정상 결과가 아니다. 명령 없는 평범한 채팅은 애초에 `valid`이므로 이 신호에 들어오지 않는다.
  - **임계값·비율을 두지 않는다.** `invalid > 0`이 이미 "우리 계약이 플랫폼을 못 읽는다"이므로 합격선을 만들 대상이 아니다. 관측 없이 숫자를 정하는 문제 자체가 사라진다.
  - `/health`의 `engine` 블록과 **로그** 양쪽에 낸다. `/metrics` 카운터만으로는 부족하다는 것이 T40이 증명한 사실이다.
  - **supervisor는 이것에 반응하지 않는다.** 재시작은 계약 불일치를 고치지 못하고, safe stop은 부분 장애(명령 불가)를 전면 장애(방송 중단)로 바꾼다 — §2.1은 시청자도 입력도 없이 세계가 진행되기를 요구한다. 사람에게 보내는 신호이므로 사람이 보는 곳에 둔다. alert sink는 웹훅이 없으면 아무 데도 가지 않으므로 같은 실수의 반복이다.
- **합격 기준**
...
## T47 — 모든 gRPC streamList 시작을 quota-safe 간격으로 pacing한다
...
- **R-T47-1 관측**: response 뒤 `UNAVAILABLE`이 오는 합법적 경로는 기존 구현에서 FakeClock starts `[0, 1000, 2000]`으로 성공-only pacing을 우회했다. 그 결과 budget test의 3,456/day 상한은 모든 실제 요청을 세지 않았고, usable headroom 검사는 `> 0`으로 약화됐으며, 설정값 `2147483648`은 Node timer overflow로 1ms에 축약될 수 있었다.
- **R-T47-2R 관측**: production `main.ts`의 chat-source restart는 같은 `ChatSource`에서 `await stop(); start()`를 호출하지만, outer `#cancelled`가 `true`로 남고 `start()`가 새 run lifecycle을 열지 않아 FakeClock을 25,000ms까지 진행해도 starts `[0]`, mode `idle`이었다. 완료된 stop 뒤 명시적 start는 sticky stop을 지우고 실제 새 run을 만들되, process component가 보유한 gRPC start pacing 시각은 유지해야 한다.
- **가정**: 결과와 무관한 **모든 실제 gRPC stream start-to-start 최소 간격**의 shipped 기본값은 **25,000ms**다. 따라서 최대 2.4 starts/minute, chat 최대 3,456 units/day이고 T44 고정 broadcast 4,608 + rollover 636과 합쳐 8,700/day다. usable 9,500에서 headroom 800은 rollover buffer 636보다 크다. 이 값은 플랫폼이 보장한 주기가 아니므로 `provisional`이며 Gate 2 fresh quota/latency evidence로 calibration한다(A-15).
- **범위**
  - normal config/env loading path에 `grpcStreamMinStartIntervalMs` / `VL_YOUTUBE_CHAT_GRPC_STREAM_MIN_START_INTERVAL_MS`를 추가하고 shipped 기본값을 25,000ms로 둔다. 호출 시작 시각부터 재므로 stream open 시간과 기존 branch backoff 시간이 interval에 포함된다.
  - floor는 response-bearing 정상 종료, response-then-error, response 0건 정상 종료, token rejection, auth/retry, retarget 뒤 첫 호출을 포함한 **모든 actual gRPC start** 경계에서 적용한다. 오류 backoff, empty-end backoff, REST `pollingIntervalMillis`, auth 분류는 추가/minimum 동작으로 그대로 유지한다.
  - pacing wait는 stop/retarget abort로 즉시 취소 가능해야 한다. synthetic/fake event를 만들지 않는다.
  - health detail은 healthy `quota_start_pacing`과 `empty_end_backoff` / `failure_backoff`를 구분한다. supervisor의 성공/실패 판정 의미는 바꾸지 않는다.
  - Node timer가 overflow되지 않도록 interval은 `2,147,483,647ms` 이하만 허용하거나 안전하게 chunking한다.
...
- **합격 기준**
  1. shipped config를 읽은 budget test가 모든 actual gRPC start의 configured floor에서 `ceil`한 worst-case daily cap을 사용하고, T44 fixed budget과 합쳐 `dailyUnits - reserveUnits` 이하이며 usable headroom이 modeled `rolloverUnits`보다 큼을 증명한다.
  2. 가상 시계 결정론 테스트가 rapid normal closes, response-then-`UNAVAILABLE`(기존 `[0,1000,2000]`), empty end, token rejection, alternating outcome, retarget 뒤 첫 call의 모든 start 간격이 configured minimum 이상임을 증명한다.
  3. 같은 테스트가 response에서 받은 token을 다음 actual request에 resume하고, quota는 actual request마다 정확히 한 번 기록함을 증명한다.
  4. stop/cancel/retarget이 pace wait 전체를 기다리지 않고 끝난다. production과 같은 동일 `ChatSource`의 완료된 `await stop(); start()`는 outer run lifecycle과 sticky stop만 초기화해 실제 재시작하고, 공유 start pacing 시각은 유지해 이전 start + floor 전에는 열지 않으며, auth/policy stop은 명시적 restart 없이 스스로 재시작하지 않는다. 오류·empty-end는 기존 backoff, REST는 서버 `pollingIntervalMillis`, auth 처리는 기존 분류를 유지하는 회귀 테스트가 녹색이다.
...
## T51 — component 재시작 완료를 정본 health 회복까지 검증한다
...
- **추가 관측**(2026-08-27 KST, 두 번째 public pilot): OBS output은 2026-08-27T12:18:37.899Z, 12:18:54.443Z, 12:18:57.996Z에 restart action 1·2·3회를 completed 처리했지만, YouTube `streamStatus`는 계속 `inactive`였다. 세 번째 action 8.799초 뒤인 12:19:06.795Z에도 `youtube_broadcast`가 degraded였고 4ms 뒤 `obs-stream:youtube_broadcast` 예산 소진으로 safe-stop했다. `ObsControl.startStream()`의 로컬 output 검증과 YouTube ingest 회복은 서로 다른 완료 경계다.
- **범위**
  ...
  - `obs-stream`은 `StartStream` RPC 성공 뒤에도 현재 장애를 만든 정본 health family가 회복될 때까지 attempt를 in-flight로 유지한다. 회복 관측 전에는 같은 degraded tick이 다음 attempt를 예약하지 못한다.
  - OBS 회복 확인 timeout은 기존 `youtube.broadcast.autoStartWaitMs + healthPollIntervalMs`를 사용한다. ingest 회복에 이미 허용한 최대 시간과 마지막 상태 poll 한 번을 포함하며 새 임계값을 만들지 않는다.
  - 회복 확인 timeout은 실패한 attempt로 기록되어 기존 backoff·bounded exhaustion을 유지하고, safe-stop/stop은 확인 timer도 취소한다.
  - T47 quota pacing·checkpoint·reconnect history, T48 safe-stop I/O halt, component restart ownership은 보존한다. live process, host task, secret, `packages/contract`, dependency, BOARD 외 운영 문서, HANDOFF는 바꾸지 않는다.
- **합격 기준**
  ...
  6. `obs-stream` action 반환 뒤 `youtube.stream_status=active`가 관측될 때까지 같은 attempt가 in-flight이며, inactive가 지속되면 `autoStartWaitMs + healthPollIntervalMs` 뒤 실패·backoff·bounded exhaustion으로 이어진다.
  7. focused 회귀 테스트 + fetch/rebase + `npm ci` + 게이트 5개 + latest-head CI가 녹색이다.

## T52 — rolling lifecycle single-flight와 다중 open attempt crash recovery

- slug `t52-rollover-singleflight` · PR 접두 `fix(youtube):` · 의존 T10, T30, T33, T45, T47, T51 · **non-contract**
- **읽을 것**: 스펙 §9.1·§9.2·§9.3·§9.4·§10.2·§11, BOARD D-21·D-25·A-18, T30, T33, T45, T47, T51, `apps/server/src/youtube/broadcast/lifecycle.ts`, `apps/server/src/db/store.ts`, `apps/server/src/youtube/chat/chat-source.ts`, `apps/server/src/main.ts`, `apps/server/src/supervisor/supervisor.ts`
- **관측**(2026-08-28 KST, exact main `a6560e46c6411ac6352f60aff821ab6b6cf12e06`, 첫 실제 11시간 rollover): supervisor의 background `rolloverIfDue()`가 replacement attempt를 만들고 있는 동안 `ChatSource.resolveTarget`이 같은 `BroadcastLifecycle.ensureBound()`를 호출했다. 두 흐름이 같은 `broadcast_resources` attempt의 `liveBroadcasts.insert`를 각각 수행해, attempt `49dbed81-e565-4200-85b4-8e27bfe6b42d`가 먼저 `zCAyrskFpx4`를 기록한 뒤 다른 흐름이 `vEI6jjv9JsA`로 repoint하려 했고 write-once guard의 `PersistenceInvariantError`가 `:8787` process를 종료했다. restart 때 이전 rollover attempt `9cd97f16-a3e5-4859-80e4-93f155c2f925`와 더 오래된 live attempt `414cb7cc-3216-4b60-91cf-f4b3b5e10d65`도 open이었다. OBS와 renderer는 계속 동작했다.
- **원인 계약**:
  - lifecycle의 read→external call→durable write sequence는 DB의 write-once guard만으로 직렬화되지 않는다. `findOpenBroadcastAttempt()`와 `pending_call`은 crash reconcile 표식이지 process 안 mutex가 아니므로, 두 caller가 같은 pre-call snapshot을 읽고 서로 다른 external result를 한 attempt에 기록할 수 있다.
  - `Supervisor.#rollingOver`는 rollover끼리만 막고 chat target resolution/startup/publish/stop 같은 다른 lifecycle mutation owner를 막지 않는다. 따라서 single-flight 경계는 supervisor나 chat이 아니라 공유 `BroadcastLifecycle` instance에 있어야 한다.
- **구현 계약**:
  - `resume`, `ensureBound`, `goLive`, `ensureLive`, `rolloverIfDue`, `publish`, `rollOver`, `stopBroadcast`처럼 open-attempt를 읽고 lifecycle/store/API를 바꿀 수 있는 모든 public 흐름은 같은 lifecycle-local asynchronous serialization boundary를 통과한다. 대기자는 선행 흐름이 성공/실패로 완전히 정리된 뒤 durable 최신 상태를 다시 읽는다.
  - composite public method가 다른 locked public method를 부르지 않는다. lock을 보유한 private 구현끼리만 조합해 reentrant self-wait/deadlock이 불가능해야 하며, 실패도 queue를 영구히 막지 않는다.
  - due rollover는 기존 순서를 그대로 지킨다: 같은 OBS ingestion stream에 private replacement create → A-18 marker 제거/bind → predecessor `complete` → configured visibility(`public`/`unlisted`) 복원 → replacement `live`. shipped `private`이면 visibility update를 생략한다. 이 replacement publish는 이미 공개된 rolling segment의 handoff에만 적용하며 초기 startup의 명시적 publish 경계를 넓히지 않는다. OBS stop/restart, world snapshot/inbox/checkpoint mutation은 없고 chat resolver는 serialization 뒤 최신 open binding을 읽어 새 `liveChatId`로 retarget한다.
  - restart는 open row 전체를 `(created_at DESC, attempt_id DESC)`로 결정론적으로 본다. newest recoverable row를 우선 candidate로 삼되, 외부 ID는 어떤 row에서도 write-once이며 다른 row로 repoint하지 않는다. 각 stale row는 exact-id `liveBroadcasts.list`의 terminal/missing evidence, 또는 lifecycle이 직접 성공/reconcile한 `complete` transition 같은 명시적 근거가 있을 때만 `complete`/`abandoned`로 닫는다.
  - newest replacement가 `bound`이고 정확히 하나의 older live predecessor가 있으면 이는 중단된 rollover로 복구한다: predecessor를 먼저 `complete`(unknown outcome이면 reconcile)하고 completion reason을 durable하게 기록한 뒤 configured visibility를 복원하고 replacement를 `live`로 만든다. predecessor가 이미 terminal이면 그 evidence와 rollover predecessor reason을 기록하고 replacement를 이어가며, 그 close 뒤 visibility/live 전에 다시 crash해도 durable reason으로 같은 replacement만 이어간다. 둘 이상의 competing live 상태(실제 incident의 두 older `live` + newest `broadcast_created` 포함), incomplete list/unknown lifecycle, binding 불일치처럼 정당 owner를 증명할 수 없으면 insert/repoint/close/publish로 추측하지 않고 기존 safe-stop/alert 경로로 올린다.
  - external resource가 없는 older planned row는 더 최신 recoverable attempt가 존재한다는 DB evidence와 pending call 부재가 함께 있을 때만 `abandoned`로 닫을 수 있다. 외부 call이 pending이거나 resource ID가 있으면 YouTube reconcile 없이 supersede하지 않는다. 이 규칙으로 restart 반복이 broadcast/stream spam을 만들지 않아야 한다.
  - supervisor background rollover promise는 모든 rejection을 관측·로그·alert하고 `PersistenceInvariantError`를 포함한 rejection이 unhandled로 process를 종료하게 두지 않는다. safe-stop 이후 outward mutation 금지와 T51 readiness/restart ownership은 유지한다.
- **보존 조건**: shipped `youtube.broadcast.strategy="rolling-experiment"`, `segmentMs=39,600,000`, simulator disabled, private insert/A-18 marker 제거 뒤 명시적 public publish, process-wide quota accounting/pacing/reserve, world-state continuity, continuous OBS ingest, T47 checkpoint/reconnect와 T51 readiness를 바꾸지 않는다. 실제 host/DB/YouTube/credential/task/log/journal, `packages/contract`, dependency, lockfile, BOARD, HANDOFF는 건드리지 않는다.
- **합격 기준**
  1. fake API의 controlled barrier로 due `rolloverIfDue()`와 concurrent `ensureBound()` 및 production과 같은 chat target resolver를 중첩한다. 두 caller가 같은 최종 replacement binding을 받고 `liveBroadcasts.insert`는 replacement당 정확히 한 번이며, attempt 하나에 두 broadcast ID를 기록하려는 경로와 `PersistenceInvariantError`가 없다.
  2. 모든 public lifecycle mutation의 단일 serialization, 실패 뒤 queue 진행, composite operation의 non-reentrant 성질을 focused test로 증명한다. rollover의 marker clear→bind→old complete→configured visibility→new live 순서, `public`/`unlisted` 복원, shipped `private` no-op, initial publish 경계, OBS 무중단, chat retarget이 회귀하지 않는다. chat concurrency와 unknown update reconcile에서도 replacement visibility update는 한 번뿐이다.
  3. exact production topology(newest `broadcast_created` + 두 older `live`)와 rollover crash boundaries(새 replacement bind 전/후, predecessor stop 전/후, visibility 복원 전/후, replacement live 전/후)를 재시작해 검증한다. newest legitimate state를 결정론적으로 resume하고, terminal/missing evidence가 있는 stale row만 reason과 함께 닫으며, ambiguous state는 resource 생성·ID rewrite·visibility mutation 없이 safe-stop/alert한다.
  4. 동일 crash/restart를 반복해도 broadcast/stream insert 수가 증가하지 않고 모든 attempt의 external ID가 안정적이다. unknown mutating call은 기존 reconcile-before-retry 규칙을 지키며 quota call은 actual request마다 기존 tracker에 한 번만 기록된다.
  5. background rollover rejection을 기다리지 않은 supervisor evaluation에서도 오류가 logger+alert에 관측되고 unhandled rejection/`PersistenceInvariantError` process crash가 없다. safe-stop 뒤 outward mutation, T47/T51 chat recovery, world snapshot/inbox/checkpoint 연속성 회귀가 없다.
  6. shipped config는 `rolling-experiment`/`39,600,000ms`, simulator off, privacy 안전을 유지하고 contract/dependency/lockfile/BOARD/HANDOFF/secret/unrelated change가 없다.
  7. `npm ci`, focused tests, `format:check`, `lint`, `typecheck`, full `test`, `build`, `soak:ci`, fetch/rebase와 exact-head CI(`soak:ci` 포함)가 녹색이다.
