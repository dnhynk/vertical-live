...
| `world_snapshot.snapshot` | current authoritative world state the renderer recovers from. Derived creature/environment/aggregate values only — the snapshot contract has no event key, broadcast id or identifier (spec §10.2, §12.4 장기 KPI는 개인 식별자가 없는 집계) | §12.4 장기 보존이 허용된 …은 30일마다 권한과 삭제 여부를 다시 확인한다. The row is rewritten on every state transition; a snapshot untouched for 30 days is reported for re-verification instead of being deleted, because deleting it would destroy the world the renderer recovers from |
