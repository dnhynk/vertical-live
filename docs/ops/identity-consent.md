...
| 사용 목적 | 화면의 '방금 반영된 행동' 슬롯에 표시명을 붙이는 것, 그리고 동의자 한정 cooldown·한 표(A-9). **그 외 용도 없음** |
...
・つかいみち: 「いま はんえいされた こうどう」の よこに なまえを だすこと。
  それ いがいには つかいません（ランキング・かきん・とうひょうの
  ゆうり ふりには つかいません）
...
- Used for: putting your name next to "the action just applied", and nothing
  else (no leaderboard, no paid perk, no vote weighting).
...
| 4 | III.E.4.b (예외 데이터의 재확인) | "the Client must still ensure every 30 days that it is still authorized by the user to access that data" | 예외 데이터도 30일마다 권한 재확인 | `world_snapshot`·`metrics_daily`는 `policy: refresh` + `reverifyPeriodDays: 30`(`config/retention.json`, `privacy/retention.ts`) | 충족 |
| 5 | **III.E.4.c** | "API Clients may store all other types of Authorized Data not identified in section (III.E.4.b) for as long as is necessary for the purposes of the specific consent granted by an active user and for no longer than **30 calendar days**." | 동의 목적에 필요한 기간, 최대 30일 | `viewer_consent.identity`: `allowedPeriodDays: 30`, `expiry: last_active_at`; 메시지마다 refresh(`db/consent.ts` `refreshConsent`); 30 초과 값은 로더가 거부(`POLICY_MAX_CONSENT_IDENTITY_DAYS`) | 충족 |
| 6 | III.E.4.d | "API Clients may temporarily store limited amounts of Non-Authorized Data ... but not longer than 30 calendar days." | 비인가 데이터 30일 | inbox·checkpoint·paid_ledger 등 모든 `authorized_api_data` 항목 30일 delete(`config/retention.json`) | 충족 |
| 7 | **III.E.4.g** | "API Clients must delete stored data as soon as possible and within **7 calendar days**" (사용자 요청·계정 삭제) | 요청 시 7일 내 삭제 | `LEAVE` 즉시 삭제(`identity/directory.ts`), 요청 handler 즉시 삭제(`privacy/deletion-request.ts`), 기록 `retention_ledger(reason=user_request\|consent_revoked, allowedPeriodDays=7)` | 충족(즉시, 7일보다 빠름) |
