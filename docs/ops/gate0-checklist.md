...
승인 전에도 구현은 진행한다. 스펙이 방향을 정한 항목은 BOARD의 가정(`A-*`)으로 두고 **플래그와 provisional
설정으로 양쪽을 구현**해 두었으므로(런북 2.5(1)), Gate 0의 답이 나오면 설정값 교체로 반영된다. 반대로 **승인 없이
채워 넣은 숫자를 합격선처럼 쓰지 않는다**(§11, BOARD A-15).
