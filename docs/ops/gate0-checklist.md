...
- [x] 채널 식별(전용 채널인가 기존 채널인가) — **기존 채널을 비워 이 프로젝트 전용으로 전환**. 승인 2026-08-22, D-10 개정
...
> 계정에 남으므로, 위 8개 항목은 **Studio(및 OAuth 연결 후 Data API)의 실제 값**으로 채운다. 국가도 실제 값으로
> 확인한다. **값과 증빙이 들어간 항목만 체크한다.**
> Google Cloud 프로젝트·OAuth 클라이언트 개설은 D-16의 사용자 작업이다.
...
- [x] **전용 채널 또는 기존 채널의 기준선·증분 수익 귀속 규칙 승인**(§1, §17) — 기존 채널을 비워 전환하므로
      **전환 시점을 기준선 0으로 보고, 그 이후의 수익 전부가 이 Live에 귀속**된다. 전환 시각과 그때의 구독자 수·
      최근 시청시간을 증빙으로 기록한다. 승인 2026-08-22, D-10 개정
