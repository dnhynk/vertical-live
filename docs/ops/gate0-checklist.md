...
> **상태: 부분 승인(2026-08-19, D-8~D-16). 잔여 3건** — §1.2 audit 값(채널 생성 후 기입), §1.4 초안 승인(T21),
> §1.7 운영 합격선(Gate 2 baseline 후 잠금). 체크되지 않은 항목은 아직 승인되지 않은 것이다.
...
### 1.2 계정 audit (§15, §8.1, §8.2, §17) — 채널 경로만 승인(D-10), **값 기입은 채널 생성 후**

YouTube Studio에서 **실제 값**을 읽어 증빙과 함께 기록한다. 이 표가 §8.1의 feature gate 입력이다([S8] [S10] [S36]).

- [x] 채널 식별(전용 채널인가 기존 채널인가) — **전용 새 채널(Brand Account)**. 승인 2026-08-19, D-10
...
> **상태(D-10, 2026-08-19)**: 전용 채널이 **아직 생성되지 않았다**. 위 8개 항목은 Studio의 실제 값이 정본이므로
> 채널 생성 후에 값과 증빙을 기입하고 그때 체크한다. D-10이 기록한 예상은 "새 채널이라 전 항목 없음/미달"
> (YPP 미가입, Supers·Gifts·Membership·Shopping 비활성, AdSense 미연결, strike 없음)이고 국가는
> **한국(가정, 채널 생성 시 확인)**이다. **예상은 증빙이 아니므로 체크박스를 닫지 않는다.**
> 채널·Google Cloud 프로젝트·OAuth 클라이언트 개설은 D-16의 사용자 작업이다.
...
- [x] **전용 채널 또는 기존 채널의 기준선·증분 수익 귀속 규칙 승인**(§1, §17) — 전용 새 채널이므로 **사전 기준선이
      없고, 이 채널의 수익 전부가 이 Live에 귀속**된다. 승인 2026-08-19, D-10
...
### 1.4 첫 화면·콘텐츠 기준 (§5.2, §6.2, §12.5, §14.2(1), §17) — **초안 대기(D-15)**
...
> **상태(D-15, 2026-08-19)**: 네 항목의 값은 **코디네이터가 초안을 제안하고 사용자가 승인**하는 절차로 정해졌다.
> 초안은 **T21**(`docs/ops/japan-panel-plan.md`)이고 숫자에는 전부 '제안' 라벨이 붙는다.
> **승인 전에는 합격선이 아니므로** 체크박스를 닫지 않는다.
...
3. 이 문서의 체크박스를 채우고 승인 날짜를 적는다. → T19에서 D-8~D-16분 완료(잔여 3건은 위 상태 문구 참조).
