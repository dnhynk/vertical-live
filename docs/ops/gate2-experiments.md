...
### 2.3 채팅은 세 표면 중 하나에만 있다 (2026-08-23 사용자 단말 실측)

| 표면 | 채팅 | CTA 실행 가능 |
| --- | --- | --- |
| 모바일 **웹** YouTube | **없음** | 불가 |
| 모바일 앱, **Shorts 피드에서 스크롤하다 만난 세로 Live** | **오버레이 없음**(확대해도 영상만) | 불가 |
| 모바일 앱, 일반 시청 페이지 | 있음 | 가능 |

**이것은 계측 제약이 아니라 제품 사실이고, 스펙의 전제 하나를 흔든다.** §5.3의 첫 화면은
"시청자가 채팅 명령으로 세계를 움직인다"를 전제로 설계돼 있는데, 세로 Live의 **주 발견 경로인
Shorts 피드에서는 그 수단이 화면에 없다.** 즉 §14.2(1)의 CTA 이해도와 `D-18`의 행동 지표는
**일반 시청 페이지에 도달한 시청자에게만 성립한다.**

따라오는 결정 두 개 — 둘 다 (2) 이후 유입 구성을 보고 사용자가 정한다.

1. 지표의 분모. Shorts 피드·모바일 웹 유입을 빼고 셀지, 표면별로 나눠 셀지.
2. 첫 화면이 무엇을 요청할지. CTA를 실행할 수 없는 표면에서도 화면이 명령을 요청하면
   그 시청자에게는 이해 실패가 아니라 **수단 부재**다 — 이해도 지표가 그것을 구분하지 못한다.

`channel_traffic_source_a3`(Reporting API)가 유입 표면을 구분하므로 (2)에서 같이 받는다.
...
- **(2)의 단말 조건 두 가지.** 둘 다 실측으로 확정된 것이지 편의가 아니다.
  ...
  - **측정은 YouTube 앱의 일반 시청 페이지에서 한다.** 채팅이 있는 표면이 거기 하나뿐이다(아래 2.3).
