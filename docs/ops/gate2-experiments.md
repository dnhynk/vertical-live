...
- **(2)의 단말 조건 두 가지.** 둘 다 실측으로 확정된 것이지 편의가 아니다.
  ...
  - **측정은 YouTube 앱에서 한다. 모바일 웹에는 채팅 칸이 자체가 없다**(2026-08-23 사용자 단말 실측). 4번 구간은 `채팅 게시 → 단말 화면`인데 모바일 웹에는 게시할 곳이 없다.

    이것은 계측 제약이 아니라 **제품 사실**이다. 세로 Live를 모바일 웹으로 만난 시청자는 화면이 요청하는 CTA를 실행할 수단이 없다 — §14.2(1)의 CTA 이해와 `D-18`의 행동 이해도 지표가 앱 시청자에게만 성립한다. 첫 화면 이해도를 판정할 때 **모바일 웹 유입 비율을 분모에서 빼거나 별도로 세지 않으면 그 지표는 틀린다.** 어느 쪽인지는 (2) 이후 유입 구성을 보고 정한다.
